...
INTERSECT		intersect_cone(t_primitive cp, t_ray r, TRANSFORM *transform)
{
	...
	float	rad = 1 + cp.data.cone.radius2;
	...
	&& intersect_test(inter.distance))))
	{
		...
		if (vec3_dot(transform->rotation, vec3_normalize(vec3_sub(inter.position, transform->position))) < 0)
			inter.intersects = 0;
		else if (cp.data.cone.size >= 0)
			inter.intersects = (vec3_distance(transform->position, vec3_add(transform->position, vec3_scale(transform->rotation, m))) <= cp.data.cone.size);
		if (inter.intersects)
